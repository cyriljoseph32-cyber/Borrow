...
function BookingRow({ b }: { b: Row }) {
  ...
  return (
    <Link
      href={`/booking/${b.id}`}
      className="flex items-center gap-3 rounded-xl border border-navy-100 bg-white p-3 transition hover:shadow-sm"
    >
      {cover ? (
      ...
      <div className="text-right">
        <Badge tone={TONE[b.status] ?? "neutral"}>{b.status.replace(/_/g, " ")}</Badge>
...
export default async function MyBookingsPage({
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-semibold text-navy-900">Bookings</h1>

      <div className="flex gap-2">
        <Link
          href="/my/bookings"
          className={`rounded-lg px-3 py-1.5 text-sm ${
            !asOwner ? "bg-navy-900 text-sand" : "border border-navy-200 bg-white text-navy-700"
          }`}
        >
          I&apos;m borrowing
        </Link>
